import { apiRequest, type JsonValue } from "../../auth/client.js";
...
export async function fetchCatalog(
  creds: Credentials,
): Promise<{ ok: boolean; tools: ToolDefinition[] }> {
  try {
    const res = await apiRequest<{ tools?: ToolDefinition[] }>(
      creds,
      "/mcp/catalogo",
      { auth: "servico" },
    );
 ...
 */
export const CATALOGO_INDISPONIVEL_TOOL: ToolDefinition = {
  name: "funilaria_catalogo_indisponivel",
  title: "Catálogo do Funilaria indisponível",
  description:
    "O backend do Funilaria não respondeu GET /mcp/catalogo, então NENHUMA tool do portal está " +
...
export async function callCatalogTool(
  ...
  return apiRequest(creds, "/mcp/executar", {
    method: "POST",
    auth: "servico",
    body: { name, args: (args ?? {}) as JsonValue },
  });
}
