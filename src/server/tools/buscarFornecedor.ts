...
const TIPOS = ["PECAS", "MATERIAIS", "AMBOS"] as const;
...
/** `GET /fornecedores` devolve o envelope paginado padrão do backend. */
...
export const BUSCAR_FORNECEDOR_TOOL: ToolDefinition = {
  name: "buscar_fornecedor",
  title: "Buscar fornecedores no diretório",
  description:
    "Lista fornecedores ATIVOS do diretório do portal (GET /fornecedores), com filtros por nome, tipo " +
  ...
  inputSchema: {
    ...
    properties: {
      ...
      type: {
        ...
        enum: [...TIPOS],
        description:
          "PECAS ou MATERIAIS filtram por especialidade e já incluem quem atende AMBOS. " +
      ...
      radiusKm: {
        ...
        maximum: 500,
        description:
          "Raio em km a partir da oficina do usuário. Só tem efeito em sessão de usuário com oficina; " +
...
export async function runBuscarFornecedor(
  creds: Credentials,
  args: unknown,
): Promise<FornecedoresResponse> {
  const a = objeto(args);
  return apiRequest<FornecedoresResponse>(creds, "/fornecedores", {
