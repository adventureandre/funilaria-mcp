...
export const IDENTIFICAR_FUNCIONARIO_TOOL = {
    name: "identificar_funcionario",
    title: "Descobrir de qual oficina é o número que mandou a mensagem",
    description: "Resolve um número de WhatsApp em funcionário e oficina " +
    ...
    inputSchema: {
        ...
        properties: {
            whatsapp: {
                type: "string",
                pattern: "^\\d{8,15}$",
                description: "Número de quem mandou a mensagem, SÓ DÍGITOS, com DDD e DDI quando houver " +
...
export async function runIdentificarFuncionario(creds, args) {
    const a = objeto(args);
    const whatsapp = textoObrigatorio(a, "whatsapp");
    return apiRequest(creds, `/estoque/integracao/identificacao?whatsapp=${encodeURIComponent(whatsapp)}`, { auth: "servico" });
}
