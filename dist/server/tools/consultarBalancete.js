...
export const CONSULTAR_BALANCETE_TOOL = {
    name: "consultar_balancete",
    title: "Balancete de consumo de materiais por período",
    description: "Resume o que a oficina consumiu, entrou e perdeu de material num período, com o custo em reais " +
    ...
    inputSchema: {
        ...
        properties: {
            shopId: {
                type: "string",
                description: "UUID da oficina. Obrigatório em chamada de serviço (o normal para a IA); omitido, vale a " +
                    "oficina da sessão de usuário, quando houver.",
            },
            from: {
                type: "string",
                description: "Início do período, ISO ('2026-08-01' ou data-hora completa). Padrão: primeiro dia do mês corrente.",
            },
            to: {
                type: "string",
            ...
            memberId: {
                type: "string",
                description: "UUID de um funcionário (ShopMember) para filtrar só os lançamentos dele. O id vem do campo " +
...
export async function runConsultarBalancete(creds, args) {
    const a = objeto(args);
    ...
    const resposta = await apiRequest(creds, "/estoque/balancete", {
