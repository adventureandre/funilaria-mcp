...
import { describeCredentials, loadCredentials, } from "../auth/credentials.js";
import { ApiError, CredentialMissingError } from "../auth/client.js";
import { ArgumentoInvalidoError } from "./tools/args.js";
import { PUBLICAR_NOTICIA_TOOL, runPublicarNoticia } from "./tools/publicarNoticia.js";
import { RESPONDER_BUSCA_PECA_TOOL, runResponderBuscaPeca, } from "./tools/responderBuscaPeca.js";
 ...
 * do servidor.
 */
async function logConfiguracao() {
    ...
    if (!resumo.temServico) {
        process.stderr.write(`[${SERVER_NAME}] Sem credencial de serviço: as tools de escrita vão recusar toda chamada. ` +
...
export async function startServer() {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        if (!tool) {
            return errorContent(new Error(`Tool desconhecida: ${name}. Disponíveis: ${[...POR_NOME.keys()].join(", ")}.`));
        }
        try {
