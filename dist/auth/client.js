...
export function requireServiceSecret(creds) {
    if (!creds.serviceSecret) {
        throw new CredentialMissingError("Nenhum segredo de serviço configurado. Rode `npx " +
            PACKAGE_NAME +
 ...
 */
function signatureHeaders(segredo, payload) {
...
function backendMessage(parsed) {
    ...
    if (parsed && typeof parsed === "object") {
        ...
        if (Array.isArray(message)) {
            const partes = message.filter((m) => typeof m === "string");
            if (partes.length > 0)
                return partes.join("; ");
        }
    }
 ...
 */
function errorFromResponse(status, parsed, auth, caminho) {
    const detalhe = backendMessage(parsed);
    const sufixo = detalhe ? ` Backend: ${detalhe}` : "";
    if (status === 400) {
        return new ApiError(`O backend recusou os dados enviados para ${caminho} (400).${sufixo} ` +
            "Corrija os campos citados e chame a tool de novo — repetir igual vai falhar igual.", status, parsed);
    }
    if (status === 401) {
        if (auth === "servico") {
            return new ApiError(`Credencial de serviço recusada em ${caminho} (401).${sufixo} ` +
                "O segredo salvo não bate com AURORA_WEBHOOK_SECRET do backend, ou a rota exige JWT de usuário. " +
                "Isso é configuração da máquina: avise o responsável, não tente outra tool.", status, parsed);
        }
        return new ApiError(`Sessão expirada ou inválida em ${caminho} (401).${sufixo} ` +
            `Rode \`npx ${PACKAGE_NAME} login\` para autenticar de novo.`, status, parsed);
    }
    if (status === 403) {
        return new ApiError(`Sem permissão para ${caminho} (403).${sufixo} ` +
            "O usuário autenticado não tem o papel exigido por esta rota.", status, parsed);
    }
    if (status === 404) {
        return new ApiError(`Recurso não encontrado em ${caminho} (404).${sufixo} ` +
            "Confira os ids enviados; não invente outro valor para tentar de novo.", status, parsed);
    }
    if (status === 429) {
        return new ApiError(`Limite de requisições atingido em ${caminho} (429).${sufixo} Espere antes de repetir.`, status, parsed);
    }
    if (status >= 500) {
        return new ApiError(`A API falhou ao processar ${caminho} (${status}).${sufixo} ` +
            "É erro do servidor, não do payload: relate ao responsável em vez de reenviar variações.", status, parsed);
    }
...
async function tryRefresh(creds) {
    ...
    if (data.user?.shopId !== undefined)
        creds.shopId = data.user.shopId ?? creds.shopId;
...
async function send(creds, caminho, opts, bearer) {
    ...
    if (opts.auth === "servico") {
        ...
        if (creds.shopId)
            headers["x-funilaria-shop"] = creds.shopId;
        ...
        if (creds.perfil)
            headers["x-funilaria-perfil"] = creds.perfil;
    ...
    try {
        ...
        throw new ApiError(`Não consegui falar com a API em ${creds.apiUrl} (${err instanceof Error ? err.message : String(err)}). Confirme se o backend está no ar e se a URL configurada está certa.`, 0);
    }
}
export async function apiRequest(creds, caminho, opts) {
    let bearer = null;
    if (opts.auth === "usuario")
        bearer = requireToken(creds);
    else if (opts.auth === "publico")
        bearer = creds.token;
    ...
    if (res.status === 401 && opts.auth !== "servico" && creds.refreshToken) {
        ...
        if (novo)
            res = await send(creds, caminho, opts, novo);
    }
    const parsed = await parseBody(res);
    if (!res.ok)
        throw errorFromResponse(res.status, parsed, opts.auth, caminho);
    return parsed;
}
