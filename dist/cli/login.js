...
export async function runLogin() {
    ...
    try {
        ...
        stdout.write(`Não consegui falar com ${apiUrl}: ${err instanceof Error ? err.message : String(err)}\n`);
        process.exitCode = 1;
        return;
    }
    ...
    if (!res.ok) {
        const detalhe = parsed && typeof parsed === "object" && typeof parsed.message === "string"
            ? parsed.message
            : `HTTP ${res.status}`;
    ...
    if (data.user?.shopId) {
        ...
        stdout.write("Atenção: este usuário não tem oficina vinculada — consultar_estoque vai falhar sem shopId.\n");
    }
